...
// sourced from: https://github.com/sipa/Coin25519/blob/master/src/crypto/ripemd160.c
...
static void ripemd160_process(ripemd160_state *self, const unsigned char *p, unsigned long length)
{
...
static void ripemd160_done(ripemd160_state *self, unsigned char *out)
{
    /* Append the padding */
...
const unsigned char *ripemd160Hash(unsigned char *buf, const unsigned char *data, int length)
{
  ripemd160_state md;
  ripemd160_init(&md);
  ripemd160_process(&md, data, length);
  ripemd160_done(&md, buf);
  return buf;
}
